...
function Customer({ open, handleClose, mode, userCode, component, handleCloseSubmit }) {
    const initialValues = {
        userId: '',
            ...
            <Dialog maxWidth="220px" open={open} keepMounted onClose={handleClose} aria-describedby="alert-dialog-slide-description">
                <DialogTitle>
                    <Box display="flex" alignItems="center" className="dialog-title">
                        <Box flexGrow={1}>
                            {mode === 'INSERT' && component === 'user_creation' ? 'Add Customer' : ''}
                            {mode === 'VIEW_UPDATE' && component === 'user_creation' ? 'Update Customer' : ''}
                            {mode === 'VIEW' && component === 'user_creation' ? 'View Customer' : ''}
